...
using UnityEngine;
//This class represents fighter 2, aka player 2.
    ...
    private void Update()
    {
        if (time <= 0)
        {
            ...
            else if (Input.GetKeyDown(jump) && extraJumps == 0 && !isGrounded && tennishit)
            {
                ...
                {
                    if (enemiesToDamage[i].name.Equals("fighter1"))
                    {
                        enemiesToDamage[i].GetComponent<PlayerController>().TakeDamage(true, facingRight, tennishit);
                    }
                }
                tennishit = false;
            ...
            if (damage == true)
            {
                ...
                for (int i = 0; i < enemiesToDamage.Length; i++)
                {
                    if (enemiesToDamage[i].name.Equals("fighter1"))
                    {
                        enemiesToDamage[i].GetComponent<PlayerController>().TakeDamage(damage, facingRight, false);
                    }
                }
            }
        }
        else
        {
            ...
            if (startTime - time > 0.2f && isGrounded)
            {
                ...
                FindObjectOfType<GameManager>().p2hurt();
                time = 0;
            }
        }
